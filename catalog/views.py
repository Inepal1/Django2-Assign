...
class BookUpdate(UpdateView):
    ...
    def form_valid(self, form):
        # Save the book instance, but don't commit yet
        post = form.save(commit=False)

        # Delete the previously stored genres for the book
        for genre in post.genre.all():
            post.genre.remove(genre)  # Remove existing genres
        ...
        for genre in form.cleaned_data['genre']:
            ...
            theGenre = get_object_or_404(Genre, name=genre)
            post.genre.add(theGenre)  # Add the genre to the book's genres

        post.save()  # Save the updated book instance with new genres
        return HttpResponseRedirect(reverse('book_list'))
...
def book_delete(request, pk):
    ...
    if request.method == 'POST' and request.user.is_superuser:
        ...
        return HttpResponseRedirect(reverse('book_list'))  # Redirect after successful deletion

    return render(request, 'catalog/book_delete.html', {'book': book})